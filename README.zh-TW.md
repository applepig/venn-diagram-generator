...
沒帶 `s` 時編輯器從空白開始：每個形狀仍有一組預設 template（依當前介面語言），但它只當輸入框的 placeholder，不寫進 state——不必先清掉範例才能寫自己的字。還沒打字時切形狀會順帶換成該 template 的樣式；打過字之後文字與樣式都不動。首頁的 og:image 仍然渲染 2 圈 template，社群卡片不會是一張空圖。
