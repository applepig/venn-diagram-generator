...
單頁 WYSIWYG 文氏圖產生器：選排列（環狀或一列）與 2～6 圈，在畫布上直接點字改字、拖動位置、調字級，一鍵下載社群可貼的 PNG。整份編輯狀態壓進網址的 `s` 參數，所以分享連結就是圖：貼進聊天軟體會透過 `og:image` 直接預覽。無登入、無帳號、server 無狀態。介面支援台灣中文、英文與日文，可從面板的語言下拉切換（記在 `venn.lang` cookie），也認 `?lang=` 與 `Accept-Language`。
