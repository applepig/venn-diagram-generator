 ...
 * 4-byte code point（U+20000 起的擴充漢字）＋各槽相異的 fs/fill，再加一個同樣塞滿的
 * 圖片標題」；槽數隨組合變，實測最大的是 ring(5) 的 21 槽 6,136 字元
 * （ring(6) 5,347、ring(4) 3,950、row(6) 3,479），所以留約 1.19 倍餘裕。
 * 掃全部 (arr, n) 的實測在 tests/state-codec.test.ts（AC15）。
 *
 * 10 sprint 移除文字位移後最壞值從 6,424 降到 6,136，但常數**刻意維持 7300 不跟著調小**：
 * 這是 HTTP 層的 DoS 閘門，鬆一點只是多解一次壓縮，改小卻可能把帶著舊 `dx`／`dy`、
 * 因而較長的舊連結擋在 decode 之前，代價是那條連結永久失效。兩邊不對等，所以留餘裕。
