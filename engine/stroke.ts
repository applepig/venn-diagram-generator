/**
 * 圓框線的解析：state 上的 `stroke_width`／`stroke` 缺席時要拿到什麼值。
...
export function defaultStrokeWidth(style: VennStyle): number {
  return STROKE_WIDTH_DEFAULTS[style];
}

/** 實際的框線寬度（畫布寬比例）；0 代表不畫框線 */
export function strokeOf(state: VennState): number {
  return state.stroke_width ?? defaultStrokeWidth(state.style);
}
